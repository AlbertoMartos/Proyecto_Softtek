...
public class ConsultaControlador {
	...
	public ResponseEntity<List<ConsultaDTO>> listar() throws Exception {
		List<ConsultaDTO> lista = servicio.listar().stream().map(p -> mapper.map(p, ConsultaDTO.class))
	...
	public ResponseEntity<List<ConsultaResumenDTO>> listarResuemn() throws Exception {
		List<ConsultaResumenDTO> lista = servicio.listarResumen();
		// ConsultaResumenDTO dtoResponse = mapper.map(lista, ConsultaResumenDTO.class);
		return new ResponseEntity<>(lista, HttpStatus.OK);
	}
